...
namespace LastFrontierApi.Controllers
{
  ...
  public class CharacterEventsController : Controller
  {
    ...
    public CharacterEventsController(LfContext context, IEventService eventService, UserManager<AppUser> userManager,
    ...
    public async Task<IActionResult> GetCharacterById(int id)
    {
      ...
      if (roles.Count == 1 && roles.FirstOrDefault() == "User")
      {
        ...
        character = _context.tblCharacter.Include(s => s.Skills)
          .FirstOrDefault(c => c.Id == id && c.PlayerId == player.Id);
      }
      else
      {
        character = _context.tblCharacter.Include(s => s.Skills).FirstOrDefault(c => c.Id == id);
      }
